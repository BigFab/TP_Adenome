...
adenome_barplot <- function(df, feature_index, title) {
  caption = "Source : ADENOME-PROS"
  x_axis = cfactolnames(df)[feature_index]
  g <- ggplot(df, aes(df[,feature_index]))
...
# On cree un dataframe ne comportant que des données numériques (pour corrélation de)
vapor_num = subset(vapor,select=-c(Comorbidite,Porteur_Sonde,Indication,Anesthesie,Evenement,Transfusion,QoL,Caillotage,Reprise_Bloc,Technique,Residu_post_mictionnel,QoL,X1M_QoL,X3M_QoL,X6M_QoL,X9M_QoL,X12M_QoL,X15M_QoL,X18M_QoL))
vapor_pre_num = vapor_num[1:9]
vapor_post_num = vapor_num[10:23]
rtupb_num = subset(rtupb,select=-c(Comorbidite,Porteur_Sonde,Indication,Anesthesie,Evenement,Transfusion,QoL,Caillotage,Reprise_Bloc,Technique,Residu_post_mictionnel,Volume_Reseque,QoL,X1M_QoL,X3M_QoL,X6M_QoL,X9M_QoL,X12M_QoL,X15M_QoL,X18M_QoL))
rtupb_pre_num = rtupb_num[1:8]
rtupb_post_num = vapor_num[9:22]
